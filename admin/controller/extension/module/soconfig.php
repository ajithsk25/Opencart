...
class ControllerExtensionModuleSoconfig extends Controller
{
    ...
    public function __construct($registry)
    {
        ...
        $this->typelayouts = array(
            ...
            'typeheader'=> array('key'=>'1', 'title'=>'Header 1 '),
    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if ($this->request->post['buttonForm'] == 'color') {
                ...
                $this->response->redirect($this->url->link('extension/module/soconfig', 'token=' . $this->session->data['token'], 'SSL'));
            } elseif ($this->request->post['buttonForm'] == 'install-data') {
        ...
        if ($table_exists) {
            foreach ($stores as $storeid => $store) {
                ...
                if (!empty($load_config)) {
                    ...
                    echo 'Error Remove module Theme Control Panel. Pleases install module Theme Control Panel';
                    exit;
                }
            }
        } else {
            echo 'Pleases install module Theme Control Panel';
            exit;
        }
