<?php
class ControllerExtensionModuleZynkCustomers extends Controller
{
    var $pricelists;
    ...
    private function _startup()
    {
        ...
        $this->zynk = new ControllerExtensionModuleZynk($this->registry);
    }
        ...
        foreach ($customers->Get() as $customer) {
            ...
            //if ($customer->AccountReference != "" && $customer->CustomerInvoiceAddress->Email != "" )
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo("Sending password to customer with account reference '$customer->AccountReference'");
                    $this->SendEmail($data);
                }
    ...
    {
        $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', AccountReference = '" . $this->db->escape($data['AccountReference']) . "', password = '" . $this->db->escape(md5($data['password'])) . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', date_added = NOW()");

        $customer_id = $this->db->getLastId();

        if ($data['newsletter']) {
            $this->db->query("UPDATE " . DB_PREFIX . "customer SET newsletter = '" . $this->db->escape($data['newsletter']) . "' WHERE AccountReference = '" . $this->db->escape($data['AccountReference']) . "'");
        }

        if ($data['customer_group_id']) {
            $this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . $this->db->escape($data['customer_group_id']) . "' WHERE AccountReference = '" . $this->db->escape($data['AccountReference']). "'");
        }
    ...
    public function EditCustomer($customer_id, $data)
    {
        ...
        if ($data['AccountReference']) {
            $this->db->query("UPDATE " . DB_PREFIX . "customer SET AccountReference = '" . $this->db->escape($data['AccountReference']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
        }
    ...
    public function AssignCustomersToPricelists(&$accounts)
    {
        foreach ($accounts->Get() as $account) {
            ...
            if ($result->num_rows > 0) {
                if (!empty($account->PriceListReference)) {
                    $customer_group = $this->pricelists->GetPricelistByRef($account->PriceListReference);

                    if ($customer_group->num_rows > 0 && $this->GetCustomerByAccountReference($account->AccountReference)->num_rows > 0) {
                        $query = $this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group->row['customer_group_id'] . "' WHERE AccountReference = '" . $this->db->escape($account->AccountReference) . "'");
                    ...
                    $query = $this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '". $this->config->get('config_customer_group_id') ."' WHERE AccountReference = '" . $this->db->escape($account->AccountReference) . "'");
                    $this->zynk->screenOutput("Updated Price List for ". $account->CustomerInvoiceAddress->Forename . " " . $account->CustomerInvoiceAddress->Surname . " - Account Reference:[".$account->AccountReference."]. </br>", 2);
                }
            }
        }
    ...
    {
        $result = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE AccountReference = '".$this->db->escape($ref)."';");
        return $result;
    }
    ...
    {
        $result = $this->db->query("UPDATE " . DB_PREFIX . "customer SET AccountReference = '" . $this->db->escape($accountReference) . "' WHERE customer_id = '" . (int)$customer_id . "'");
        return $result;
    }
