...
class ControllerExtensionModuleZynk extends Controller
{
    ...
    private function _startup()
    {
        ...
        */
        ...
        if (! $this->config->get('module_zynk_status')) {
            die('Module is not enabled.');
        }

        // check that the bank account is set
        if ($this->config->get('module_zynk_download_payments') & !$this->config->get('module_zynk_bank_account')) {
        ...
        if (!$this->config->get('module_zynk_download_customers') & !$this->config->get('module_zynk_account_reference')) {
        ...
        include(DIR_APPLICATION.'../admin/model/customer/customer.php');
        ...
        $this->model_customer_customer  = new ModelCustomerCustomer($this->registry);
        ...
        $this->products     = new ControllerExtensionModuleZynkProducts($this->registry);
        $this->pricelists   = new ControllerExtensionModuleZynkPricelists($this->registry);
        $this->customers    = new ControllerExtensionModuleZynkCustomers($this->registry);
    }
    ...
    public function download()
    {
        ...
        if (!empty($post)) {
            ...
            foreach ($this->devkit->Invoices->Get() as $invoice) {
                ...
                (
                    'notify'            => false,
                    'order_status_id'   => $this->config->get('module_zynk_notify_stage'),
        ...
        } elseif ($this->config->get('module_zynk_download_orders') or $this->config->get('module_zynk_download_products')) {
            if ($this->config->get('module_zynk_download_orders')  & !isset($_GET['products'])) {
                // Limit by OrderID
                if (isset($_GET['order_id'])) {
                    ...
                    $data = array
                    (
                        'filter_order_status_id'    => $this->config->get('module_zynk_download_stage'),
                        'limit'                     => $this->config->get('module_zynk_download_limit'),
                        'start'                     => 0
                    );
                    
                    //print_r($this->config->get('module_zynk_download_stage'));
                ...
                $type = ($this->config->get('module_zynk_download_type') == 'invoice') ? 'Invoice' : 'SalesOrder';
            ...
            if ($this->config->get('module_zynk_download_products') & isset($_GET['products'])) {
                // Limit by OrderID
    ...
    public function upload()
    {
        ...
        if (!empty($post)) {
            ...
            if ($this->config->get('module_zynk_upload_products')) {
            ...
            } elseif ($this->config->get('module_zynk_upload_product_quantities')) {
                $this->screenOutput("</br></br><b>Products</b></br>", 2);
                $this->products->updateStockLevel($this->devkit->Products);
            }
            
            // Create or Update Product Quantity Price Breaks
            if ($this->config->get('module_zynk_upload_product_price_breaks')) {
            ...
            if ($this->config->get('module_zynk_upload_product_special_offer')) {
    ...
    private function download_order($order_id, $order_type = 'SalesOrder')
    {
        ...
        if ($this->config->get('module_zynk_vat_inclusive_prices')) {
        ...
        $sales_order->TakenBy                   = $this->config->get('module_zynk_taken_by');
        ...
        if ($this->config->get('module_zynk_shipping_as_item_line')) {
        ...
        if ($coupons->num_rows > 0) {
            foreach ($coupons as $coupon) {
                if ($sales_order->NetValueDiscountDescription != "") {
                    $sales_order->NetValueDiscountDescription.", ";
                }
                $sales_order->NetValueDiscountDescription .= $coupon['name'];
                $sales_order->NetValueDiscount += abs($coupon['amount']) / 1.2; /* / (1 + ($this->config->get('module_zynk_default_tax_rate') / 100))*/
                //$couponAmount += abs($coupon['amount']);
            }
        }

        // payment details
        if ($this->config->get('module_zynk_download_payments')) {
            ...
            $sales_order->BankAccount           = $this->config->get('module_zynk_bank_account');
        }
        ...
        if ($this->config->get('module_zynk_download_customers')) {
            $this->download_customer($order['customer_id'], $sales_order_address, $sales_delivery_address, $sales_order->{$order_type.'Date'});
        } else {
            $sales_order->AccountReference = $this->config->get('module_zynk_account_reference');
        }
    }
    ...
    private function get_order_voucher_payment_comment_line($voucher)
    {
        ...
        //    $sales_order->NetValueDiscount += $giftVoucherTotal  / (1 + ($this->config->get('module_zynk_default_tax_rate') / 100));
    ...
    private function add_order_voucher_payment_transactions($voucher, $orderId, $date)
    {
		...
		$jcTransaction->AccountReference = $this->config->get('module_zynk_account_reference');
        ...
        $jdTransaction->AccountReference = $this->config->get('module_zynk_account_reference');
        ...
        $jdTransaction->NominalCode = 2101;//$this->config->get('module_zynk_bank_account');
        $jdTransaction->BankReference = 1205;//$this->config->get('module_zynk_bank_account');
    ...
    /*private function add_order_voucher_payment_transactions_when_used($voucher, $orderId, $date)
	{
        ...
        //$this->config->get('module_zynk_bank_account')
    }*/
...
//        $item->UnitPrice    = $shippingNet['value'];
        $item->TaxRate      = ($ShippingTaxCode > 0) ? $this->config->get('module_zynk_default_tax_rate') : 0;
        $item->TaxCode      = ($ShippingTaxCode > 0) ? $this->config->get('module_zynk_default_tax_code') : 0;
        $item->NominalCode  = 4905;
...
//                    if( $this->config->get('module_zynk_download_cost_price') )
//                    {
...
//            $cust->AccountReference     = $customer['AccountReference'];
            ...
            $cust->AccountReference     = $this->config->get('module_zynk_account_reference');
    ...
    public function GetTaxCode($country_code)
    {
        ...
        $TaxCode_UK     = $this->config->get('module_zynk_default_tax_code');
        $TaxCode_EU     = 1;
        $TaxCode_ROW    = 0;

        $taxCode        = $this->config->get('module_zynk_default_tax_code');
    ...
    public function TaxClassToTaxCodeMap($tax_class_id)
    {
        ...
        switch ($tax_class_id) {
            case $this->config->get('module_zynk_vatable_taxclass'):
                $taxCode = $this->config->get('module_zynk_vatable_taxcode');
                break;
            case $this->config->get('module_zynk_nonvatable_taxclass'):
                $taxCode = $this->config->get('module_zynk_nonvatable_taxcode');
                break;
            case $this->config->get('module_zynk_exempt_taxclass'):
                $taxCode = $this->config->get('module_zynk_exempt_taxcode');
                break;
            default:
                $taxCode = $this->config->get('module_zynk_nonvatable_taxcode');
                break;
