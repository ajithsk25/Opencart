...
class ControllerExtensionModuleZynkCategories extends Controller
{
    public function CreateAndAssignCategory($sage_product, $product_id)
    {
        ...
        //$this->SetCategory($product_id, 34);
    }
    ...
    public function CreateAndAssignDefaultCategory($sage_product, $product_id)
    {
        try {
            ...
            $category = $this->config->get('module_zynk_default_category');
            if ($category > 0) {
                $this->SetCategory($product_id, $category);
            }

            //$Category_Name = $this->config->get('module_zynk_default_category');
    ...
    {
        $data['sort_order']         = ($this->config->get('module_zynk_upload_categories')) ? 0 : -1; //-1: Hide from listing...
        $data['image']              = '';
        ...
        $data['status']             = 1;//($this->config->get('module_zynk_upload_categories')) ? 1 : 0;
