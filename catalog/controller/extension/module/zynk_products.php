...
class ControllerExtensionModuleZynkProducts extends Controller
{
    var $categories;
    ...
    private function _startup()
    {
        ...
        $this->Categories = new ControllerExtensionModuleZynkCategories($this->registry);
    ...
    public function UpdateProducts(&$products)
    {
        ...
        foreach ($products->Get() as $product) {
            // Check if the Sku for this product exists
            // in the product options table. If it doesn't,
            // assume it's a standalone product.
            if ($this->IsProductOption($product)) {
                $this->UpdateProductOption($product);
            } else {
                $this->SetProduct($product);
            }

            //RFC: 18/07/2012: J&SC want products to not be visible on the website once they are not ticked publish to web
    ...
    public function SetProduct($product)
    {
        ...
        $dbProd = $this->db->query("SELECT product_id, sku FROM ".DB_PREFIX."product WHERE sku = '$product->Sku';");
        ...
        //$uos = mysql_real_escape_string(htmlentities($product->UnitOfSale, ENT_QUOTES));

        $status = $this->getStatus($product);

        if ($this->config->get('module_zynk_upload_product_quantities')) {
        ...
        //$tax_class_id = ($product->TaxCode == 0) ? $this->config->get('module_zynk_nonvatable_taxclass') : $this->config->get('module_zynk_vatable_taxclass'); // Client specific, default to UK Vatable
        //$query .= "tax_class_id     = '" . $tax_class_id ."', ";    // Taxable
        $query .= "tax_class_id = '".$this->GetTaxClass($product->TaxCode)."', ";
        ...
        $query .= "status           = '$status' ";
        //RFC-31/05/2012 - store unit of sale in new column on website so that adam can display with the product names
        // not needed for standard Opencart
        //$query .= ", unit_of_sale		= '$uos'";

        if ($this->config->get('module_zynk_upload_product_images')) {
            $imageDirectory = $this->config->get('module_zynk_default_product_image_directory');

            if (!empty($product->ImageUrl)) {
                print_r($product->ImageUrl);
                // Write image to file
                $imageData  = base64_decode($product->ImageUrl);
                $handle     = fopen(DIR_APPLICATION.$imageDirectory.'/'.$product->ImageUrl, "w");
                ...
                if ($this->DoesImageExist($product->ImageUrl)) {
                    $query .= ", image = '". $imageDirectory . "/" . $product->ImageUrl ."' ";
        ...
        if ($this->config->get('module_zynk_upload_cost_price')) {
            // Update the Cost Price
            $query .= ", cost_price = '".$product->LastCostPrice."' ";
        }
...
*/
        ...
        if ($this->config->get('module_zynk_upload_categories')) {
    ...
    {
        return file_exists("./image/".$this->config->get('module_zynk_default_product_image_directory')."/$product_image");
    }
    ...
    public function HasMultipleImages($product_sku)
    {
        ...
        if ($p->num_rows > 0) {
            ...
            $result = $this->db->query("SELECT * FROM ".DB_PREFIX."product_image WHERE product_id = '".$id."';");
    ...
    public function AssignMultipleProductImages($product_image, $product_id)
    {
        ...
        foreach (glob("./image/".$this->config->get('module_zynk_default_product_image_directory')."/$product_image-*.*") as $filename) {
    ...
    {
        $rows = $this->db->query("SELECT * FROM " .DB_PREFIX. "product_description WHERE product_id = ".$product_id." AND language_id = 1;");
        $name = $product->Name;
        //RFC-22/05/2012 - Some products have the same name but different units of sale, Hannah would like to see this
        //on the main product list in open cart
        //RFC-31/05/2012 - Hannah didn't like unit of sale coming down into sage on the order, Adam N adding unit_of_sale column to product table and displaying this through the website
        /*$uos = mysql_real_escape_string(htmlentities($product->UnitOfSale, ENT_QUOTES));
		if (!empty($uos))
		{
			$name .= " - " . $uos;
        ...
        if ($this->config->get('module_zynk_upload_product_descriptions')) {
            if (!empty($product->LongDescription) || strlen($product->LongDescription) > 0) {
        ...
        if ($rows->num_rows > 0) {
            if ($this->config->get('module_zynk_upload_product_descriptions')) {
                $query = sprintf("UPDATE " .DB_PREFIX. "product_description SET name = '%s',  description = '%s', language_id = 1 WHERE product_id = %d AND language_id = 1;", $name, $long, $product_id);
                $this->db->query($query);

                echo("Updated Product Description for ". $name . " - SKU:[".$product->Sku."]. </br>");
            } else // Only update name
            {
                $query = sprintf("UPDATE ".DB_PREFIX."product_description SET name = '%s', language_id = 1 WHERE product_id = %d AND language_id = 1;", $name, $product_id);
                $this->db->query($query);

                echo("Updated Product Name for ". $name . " - SKU:[".$product->Sku."]. </br>");
            }
        } else {
            $query = sprintf("INSERT INTO " .DB_PREFIX. "product_description (product_id, language_id, name, description) VALUES (%d, %d, '%s','%s');", $product_id, 1, $name, $long);
            $this->db->query($query);
    ...
    public function IsProductOption($product)
    {
        $result = $this->db->query("SELECT `product_option_id` FROM  " . DB_PREFIX . "product_option_value WHERE product_id = '".$product->Id."';");
...
//echo("query: $query </br>");
    ...
    // Updating product options (no adding of options like this)
    // At the minute, can only update stock qty
    // TODO: Allow updating of price from Sage
    //////  - As OC uses + or - from main product price. Main product would always have to be updated before its options.
    // TODO: Allow updating of weight from Sage
    public function UpdateProductOption($product)
    {
        $query  = "UPDATE ".DB_PREFIX."product_option_value SET `quantity` = '$product->QtyInStock' WHERE `sku` = '$product->Sku';";
        $result = $this->db->query($query);
        echo("Updated quantity for Option Sku:[".$product->Sku."] to: " . $product->QtyInStock . ". </br>");
    }
    ...
    public function GetTaxClass($TaxCode)
    {
        // Assign the correct taxclass to the product in Opencart
        // based on what was sent up from Sage.
        $tax_class = $this->MatchTaxClass($TaxCode);


        if (!$tax_class) {
            // The taxcode applied to the product in Sage didn't match
            // any tax classes in Opencart. Apply the default tax code
            // to the product and find which tax class goes with that.
            $TaxCode = $this->config->get('module_zynk_default_taxcode');
            // We also provide the optional $default parameter to
            // make sure that if the default tax code isn't found
            // the product will still be given a tax class ID
            // (hopefully this would never actually happen)
            $tax_class = $this->MatchTaxClass($TaxCode, 1);
        }
    ...
    public function updateStockLevel($products)
    {
        echo("got here");
        $this->_startup();
        foreach ($products->Get() as $product) {
            $free_stock = ((int)$product->QtyInStock - (int)$product->QtyAllocated);
            // Check if the Sku for this product exists
            // in the product options table. If it doesn't,
            // assume it's a standalone product.
            $oc_product = $this->FindProductBySku($product->Sku);
            if ($oc_product->num_rows > 0) {
                $result = $this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = '$free_stock' WHERE sku = '$product->Sku'");
                echo "Product " . $product->Sku . " quantity updated to " . $free_stock;
            } else {
                echo 'Product or product option not found. Nothing will be updated.';
            }
        }
    }

    private function MatchTaxClass($TaxCode, $default = null)
    {
        switch ($TaxCode) {
            case $this->config->get('module_zynk_vatable_taxcode'):
                    $tax_class = $this->config->get('module_zynk_vatable_taxclass');
                break;
            case $this->config->get('module_zynk_nonvatable_taxcode'):
                    $tax_class = $this->config->get('module_zynk_nonvatable_taxclass');
                break;

            case $this->config->get('module_zynk_exempt_taxcode'):
                    $tax_class = $this->config->get('module_zynk_exempt_taxclass');
                ;
                break;
